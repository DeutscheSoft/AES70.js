 ...
 */
export declare class OcaFirmwareManager extends OcaManager {
  /**
   * This event is emitted whenever ComponentVersions changes.
   */
  OnComponentVersionsChanged: PropertyEvent<OcaVersion[]>;
   ...
   * image to the firmware manager. The actual firmware manager implementation
   ...
   * @returns {Promise<void>}
   */
  BeginActiveImageUpdate(component: IOcaComponent): Promise<void>;

  /**
   * Adds a new part of the software/firmware image to the upgrade memory as
   ...
   * In the function the component type, details of the server and the filename
   * of the file containing the component software/firmware image needs to be
   * passed. The device will try to retrieve the new software/firmware image
   * from the server and update its component using this image. The actual
   * method for retrieving the image (e.g. TFTP) and the underlying update
   * technique (e.g. netflash) depend on the implementation and may differ
   * between components. Just the interface is standardized.
   *
   ...
   * updated (actively or passively). This action will trigger the device to
