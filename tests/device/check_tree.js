  ...
  async check_object(o, parent, i) {
    if (Array.isArray(o)) {
      ...
      try {
        ...
        this.check(
          ono === parent.ObjectNumber,
          'object %O has owner property %o (should be %o)',
          o.ObjectNumber,
          ono,
          parent.ObjectNumber
        );
      ...
      this.check(
        o.ObjectNumber >= 4096,
        'object has reserved ObjectNumber ( below 4096 )'
      );
      ...
      try {
        ...
        this.check(
          id.ClassID.startsWith(o.ClassID),
          'ClassID does not match in object %o (%o vs. %o)',
          o.ObjectNumber,
          id.ClassID,
          o.ClassID
        );
...
class CheckPath extends ObjectTest {
  ...
  async check_object(o, parent, i) {
    ...
    this.check(
      !roles.has(role),
      'Role name collision with object %d (role: %o)',
      o.ono,
      role
    );
  ...
  async check_object(o, parent, i) {
    ...
    this.rolemap.forEach((tmp, path) => {
      if (o === tmp) {
        ...
        this.check(
          path.split('/').pop().startsWith(role),
