(function(OCA) {
 ...
 Connection.prototype = {
    ...
    read: function(buf) {
        var ret = [];
        var pos = OCA.decodeMessage(new DataView(buf), 0, ret);

        if (pos != buf.byteLength) console.warn('data lost');
        ...
        for (i = 0; i < a.length; i++) {
            ...
            if (o instanceof OCA.Response) {
                ...
                if (o.status_code !== 0) {
                    ...
                    var args = (o.param_count && h[0]) ? h[0].low_decode(new DataView(o.parameters)) : [];
                    h[1].apply(window, args);
                }
 ...
 Command.prototype = {
     ...
     decode_from: function(data, pos, len) {
        ...
        if (len > 0) {
            if (!this.param_count) throw("Bad Command.");
 ...
 Response.prototype = {
     ...
     decode_from: function(data, pos, len) {
        ...
        if (len > 0) {
            if (!this.param_count) throw("Bad Response");
            this.parameters = data.buffer.slice(data.byteOffset+pos, data.byteOffset+pos+len);
            pos += len;
        }
        return pos;
 ...
 Notification.prototype = {
     ...
     decode_from: function(data, pos, len) {
        ...
        if (len > 0) {
            if (this.param_count <= 1) throw("Bad Command.");
            this.parameters = data.buffer.slice(data.byteOffset+pos, data.byteOffset+pos+len);
            pos += len;
        }
        return pos;
 ...
 function decodeMessage(data, pos, ret) {
     ...
     if (message_offset > data.byteLength)
        throw("EOF");
