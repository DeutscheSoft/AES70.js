...
var f = function(w) {
 ...
 var utf8_to_buffer, buffer_to_utf8;
 var encoder, decoder;

 if ('TextEncoder' in w && 'TextDecoder' in w) {
   encoder = new TextEncoder();
   decoder = new TextDecoder();
   buffer_to_utf8 = decoder.decode.bind(decoder);
   utf8_to_buffer = encoder.encode.bind(encoder);
 } else if ('StringDecoder' in w) {
   /* This is only for nodejs */
   decoder = new w.StringDecoder();
   buffer_to_utf8 = function(b) {
     return decoder.end(Buffer.from(b));
   };
   ...
   utf8_to_buffer = function(b) {
     return string_to_buffer(encode_utf8(b));
   };
 }
 ...
 function signature() {
     ...
     for (i = 0; i < arguments.length; i++) {
         ...
         case "object":
             ...
             break;
         default:
             throw new Error("Bad argument: " + itypeof(arguments[i]));
         }
     }
 ...
 function low_encoded_length(src, src_pos) {
     ...
     for (num_custom = 0, i = 0; i < S.length; i++, src_pos++) {
         ...
         case PT_BLOBFIXED: {
             ...
             break;
         case PT_STRING:
             pos += 2 + utf8_to_buffer(src[src_pos]).byteLength;
             break;
         case PT_BITSTRING: 
 ...
 function LISTFIXED(length, type) {
